...
def take_remote_snapshot(host, finput):
 print 'loading  in = ' + host
 ...
 driver1.get(url1)
 driver2 = webdriver.Remote(host, webdriver.DesiredCapabilities.FIREFOX)
 driver2.get(url2)
 ...
 timestamp = date('%Y-%m-%d-%H-%M-%S')
 f.write('\n' + dir + '/a-' + ts + '.png' + ' ' + url1)
 f.write(dir + '/b-' + ts + '.png' + ' ' + url2)
 driver1.get_screenshot_as_file(dir + '/a-' + ts + '.png')
 driver2.get_screenshot_as_file(dir + '/b-' + ts + '.png')
 driver1.quit()
 driver2.quit()
...
def take_5_snapshots(host, l_input, dir):
  ...
  for list in lol:
   for f in list:
    ...
    if os.path.isfile(f_path):
     p = Process(target=take_remote_snapshot, args=(host, f_path,))
     p.start()
   p.join()
 else:
...
def main():
 ...
 #chrome0 = "http://im1a10.internetmemory.org:5557/wd/hub"
...
#check options
 for o,a in opts:
  if o in ('-h', '--help'):
   ...
   ts = date('%Y-%m-%d-%H:%M:%S')
   #log.write('\n' + str (ts) + ' starts taking snapshots')
   #log.close()
   if os.path.isdir(a):
    print 'is a dir'
    ...
    if (n > 5):
     ...
     for l in lol:
      ...
      if iteration == 0 :
       print 'node0 = ' + str(l)
       p0 = Process(target=take_5_snapshots, args=(node0, l, a,))
       p0.start()
      if iteration == 1 :
       print 'node1 = ' + str(l)
       p1 = Process(target=take_5_snapshots, args=(node1, l, a,))
       p1.start()
      if iteration == 2 :
       print 'node2 = ' + str(l)
       p2 = Process(target=take_5_snapshots, args=(node2, l, a,))
       p2.start()
      iteration += 1
   else:
